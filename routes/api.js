...
function normalizeMetaPhone(phone) {
  ...
  return digitsOnly ? hashMetaField(digitsOnly) : undefined;
}

router.post("/meta-capi", async (req, res) => {
  try {
    ...
    if (!accessToken || accessToken === 'YOUR_META_CAPI_ACCESS_TOKEN_HERE') {
      return res.status(200).json({ success: false, message: 'META_CAPI_ACCESS_TOKEN missing in environment' });
    }

    const clientIp = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.ip || undefined;
    const userAgent = req.headers['user-agent'] || undefined;

    const processedUserData = {
      ...(clientIp && { client_ip_address: clientIp }),
      ...(userAgent && { client_user_agent: userAgent }),
      ...(user_data.em && { em: [hashMetaField(user_data.em)] }),
      ...(user_data.ph && { ph: [normalizeMetaPhone(user_data.ph)] }),
      ...(user_data.fn && { fn: [hashMetaField(user_data.fn)] }),
      ...(user_data.ln && { ln: [hashMetaField(user_data.ln)] }),
    };
